...
자바스크립트에서 데이터를 담고 사용하는데 이용되는 배열과 이를 loop으로 처리하기 위해서 `forEach()`와 `map()`을 주로 사용한다. 둘 다 순회 메소드이다. 
